...
def overview_update(week_range, services, events, store):
    df = _filter_services(_services_df, week_range, services, events)
    ...
    refused = df_sw.groupby("week", as_index=False)["patients_refused"].sum().sort_values("week") if "patients_refused" in df_sw.columns else pd.DataFrame({"week": [], "patients_refused": []})
    fig_ref = px.bar(refused, x="week", y="patients_refused", labels={"week": "Week", "patients_refused": "Refused (sum)"})
    fig_ref.update_layout(margin=dict(l=10, r=10, t=10, b=10))
...
def overview_hover_preview(hoverData, week_range, services, events):
    ...
    # pick a point that has x/y/z (avoid strip/outline weirdness)
...
def overview_lock_selection(clickData, store):
    ...
    # Only lock when the clicked point is a heatmap cell (has x/y/z)
    p = next((q for q in pts if ("x" in q and "y" in q and "z" in q)), None)
    if p is None:
        return store
